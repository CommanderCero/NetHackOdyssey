...
def add_altorg_directory(path, name, subfolders: List[str], filename=nld.db.DB):
    ...
    with nld.db.db(filename=filename, rw=True) as conn:
        ...
        for subfolder in subfolders:
            for ttyrec in glob.iglob(os.path.join(path, subfolder, "*", "*.ttyrec.bz2")):
                parts = Path(ttyrec).parts
                if str(Path(*parts[-2:])) in blacklisted_ttyrecs:
                    continue
                ttyrecs_dict[parts[-2].lower()].append(ttyrec)
